...
- **WCAG 2.2** (W3C Recommendation) — the criteria this skill's checklist enforces are 1.4.1, 1.4.3 and 1.4.11, cited inline where they bite; the shared floor with full definitions is in `ui-craft` §The accessibility floor. What is specific to this skill: **1.4.3** applies to text over images, video and gradients with **no exemption**, and is evaluated against the worst-case background anywhere under the text rather than an average — which is why move 10 solves for a minimum alpha instead of eyeballing a scrim; **1.4.6** is the AAA tier at 7:1; and WCAG defines no measurement method for `text-shadow` at all, which is why a text shadow can improve legibility but can never be the guarantee.
...
- **`backdrop-filter`** — Baseline *newly available* since September 2024; backdrop-root establishment by ancestors with `opacity < 1`, `filter`, `mask`, `clip-path`, `mix-blend-mode`, `backdrop-filter`, or `will-change` for those (MDN). **`prefers-reduced-transparency`** — Chrome/Edge 118+ and Samsung Internet 25+, which caniuse put at ~73% of global usage in August 2026; Firefox has implemented it but ships it disabled by default; Safari and iOS Safari do not support it. So the query reports the *supported* majority and stays silent for roughly a quarter of traffic — enhancement only, never the guarantee.
...
- `references/elevation-tokens.md` — read when setting up or migrating a project: how to choose between the three system styles, complete light and dark token files for the two that are not the default (flat/bordered, dark-first) with the soft-shadow product default staying in move 4 above, a per-component elevation assignment table, a comparison against Material and Tailwind's default `shadow-*` scale, and a lint rule plus staged migration for a codebase already full of bespoke shadows.
