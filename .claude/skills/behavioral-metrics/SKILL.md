...
**Multiple comparisons.** Twenty metrics at α=0.05 gives you roughly a one-in-a-thousand chance that *none* of them shows a spurious "significant" result — so on a typical dashboard, something always wins. Designate one primary metric; treat everything else as exploratory and say so out loud; apply a correction (Benjamini–Hochberg for a family of metrics you actually care about) if you must make claims about several. The same arithmetic applies to running many variants and to slicing by segment.
