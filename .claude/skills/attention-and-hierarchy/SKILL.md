...
description: Decides where the eye lands on a screen and makes that the place that matters — preattentive salience, Gestalt grouping, whitespace, contrast, scan patterns, and motion budgets. Use this whenever the user is laying out or reviewing any screen, page, dashboard, card, empty state, email or component, even if they never say "hierarchy" or "attention" — including requests like "make this look cleaner", "this feels cluttered/busy/overwhelming", "nobody clicks the button" and you suspect nothing on the screen tells them it matters, "people miss the banner", "which CTA should be primary", "our dashboard is unreadable", or "does this pass accessibility" where the question is legibility and rank — the contrast, target-size and focus *values* are `color-and-theming`, `spacing-and-layout` and `ui-signifiers-and-states`. Also use it before any visual-design review, and any time a screen has grown more than one prominent call to action. On a table or work queue this owns the salience pass only; what the list itself does — default sort, which columns earn a place, row actions, bulk selection — is `list-and-queue-design`.
---
